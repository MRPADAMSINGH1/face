...
from .forms import FaceRegistrationForm
...
def register_face(request):
    if request.method == 'POST':
        form = FaceRegistrationForm(request.POST, request.FILES)  # Include request.FILES here
        if form.is_valid():
            ...
            # Save the form to dataset
            form.save()
            return render(request, 'success.html')  # Replace with your actual success URL
    else:
        # Create a new form instance
        form = FaceRegistrationForm(initial={
...
@login_required
def face_registration_detail(request):
    try:
        face_registration = get_object_or_404(FaceRegistration, user=request.user)
    except Http404:
        messages.error(request, "No face registration found for the logged-in user.")
        return redirect('/')  # Change 'home' to the name of your desired redirect URL

    return render(request, 'face_registration_detail.html', {'face_registration': face_registration})

def success_page(request):
    return render(request, 'success.html')

def generate_private_key(username):
    unique_str = f"{username}{uuid.uuid4()}"
